...
def make_connection(settings, access_token):
    print(access_token)
    connectionOk = False
    while not connectionOk:
        try:
...
def main(args):
    ...
    with codecs.open(ninja_path, 'w', 'utf-8') as ninjafile:
        ...
        # get file command
        get_cmd_path = Path(os.path.relpath(str(Path.cwd()), str(ninja_path.parent)), 'boxninja_pget.py')
        print(get_cmd_path)
        ...
        recurse_boxfolders(foldermap, nj, nj_getfile, nj_checkfile, args, settings, client, ninja_path)
